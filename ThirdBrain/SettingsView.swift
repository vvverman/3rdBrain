    ...
    var body: some View {
        Form {
            ...
            Section("Хранение и приватность") {
                ...
                Text("Файлы Whisper и его токенизатора загружаются с Hugging Face только после подтверждения. Аудио и заметки не отправляются внешней LLM или серверу распознавания. После установки Whisper работает без сети.").font(.footnote).foregroundStyle(.secondary)
            }
            Section("О приложении") {
                LabeledContent("3rdBrain", value: "0.1.0")
                Text("SwiftUI · SwiftData · AVFoundation · SpeechAnalyzer · WhisperKit · Foundation Models").font(.footnote).foregroundStyle(.secondary)
