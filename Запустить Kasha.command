#!/usr/bin/env bash
# Полный первый запуск на Mac. Данные заметок и модели лежат вне репозитория.
set -euo pipefail
cd "$(dirname "$0")"
...
trap failed ERR
if [ "$(uname -s)" != Darwin ]; then echo "Для Linux используйте bash scripts/setup-models.sh и bash run-web.sh."; exit 1; fi
PACKAGES=""
