...
hdiutil create -volname '3rdBrain Test' -srcfolder "$STAGE" -ov -format UDZO "$OUT/3rdBrain-Test-1.1.0-macOS-arm64.dmg"
hdiutil verify "$OUT/3rdBrain-Test-1.1.0-macOS-arm64.dmg"
mkdir -p "$OUT/mounted"
hdiutil attach -nobrowse -readonly -mountpoint "$OUT/mounted" "$OUT/3rdBrain-Test-1.1.0-macOS-arm64.dmg"
...
(cd "$OUT" && shasum -a 256 3rdBrain-Test-1.1.0-macOS-arm64.dmg > SHA256SUMS.txt)
python3 - <<'PY'
import pathlib,json,platform
p=pathlib.Path('studio-output');dmg=p/'3rdBrain-Test-1.1.0-macOS-arm64.dmg'
