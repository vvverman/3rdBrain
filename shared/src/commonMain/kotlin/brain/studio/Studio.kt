...
/** Расширение существующих портов. Имя updateCaptureDraft сохранено только для совместимости формата/API. */
...
/** Выбор реализации не находится в интерфейсе: тестовый пакет всегда использует DemoIntelligence. */
...
class DemoIntelligence(private val latencyMillis: Long = 450) : Intelligence {
    ...
    override suspend fun tidy(text: String, language: String): String {
        delay(latencyMillis)
        // Это демонстрационный фильтр, не языковая модель и не обещание обработки любых форм мата.
        val replacements = mapOf("какая-то фигня" to "проблема", "эта хрень" to "эта функция", "фигня" to "проблема",
