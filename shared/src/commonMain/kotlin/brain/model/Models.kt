...
@Serializable data class RuntimeStatus(val whisperConfigured: Boolean = false, val llmConfigured: Boolean = false, val localOnly: Boolean = true, val message: String = "", val simulated: Boolean = false)
@Serializable data class AppSnapshot(val projects: List<Project> = emptyList(), val notes: List<Note> = emptyList(), val captures: List<Capture> = emptyList(), val runtime: RuntimeStatus = RuntimeStatus())
