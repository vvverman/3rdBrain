...
class StudioProcessingTest {
    @Test fun settingsPersistAndValidate()=runBlocking{
    ...
    @Test fun realAudioWithMockIntelligenceAndNoModels()=runBlocking{
        ...
        try{
            ...
            val processor=StudioProcessor(store,prefs,DemoIntelligence(0),"ffmpeg")
            val repo=StudioDiskRepository(store,processor,prefs,scope)
            val c=repo.createDemo()
            withTimeout(15000){while(store.capture(c.id)!!.status.isWorking)delay(50)}
            ...
            val project=repo.createProject(ProjectDraft("Приложение"));repo.distribute(c.id,DistributionRequest(project.id))
    ...
    @Test fun encoderFailurePreservesSource()=runBlocking{
    ...
    @Test fun singleCurrentEnforcedByStoreNotOnlyUi()=runBlocking{
    ...
    @Test fun discardFailureRollsBackAudioMove()=runBlocking{
