...
// LEVEL UP — the meter charges from empty. All four level bars reset to nothing,
// then each pops up from the base in turn, left → right, with a little overshoot
// as it lands.
//
// The four bars are the glyph's own subpaths (CASE + BARS + NUB recompose the
// original exactly), so nothing is added and nothing is filled — the animation
// only moves existing ink. The stagger lives in each bar's `times`, NOT a
// `delay`: a delay would hold the bar full until its slot then snap to empty
// (backwards). Encoding the hold-at-zero in the keyframes keeps every bar empty
// from the first frame until its turn. Base-anchored, so bars grow up from the
// bottom like a real level.
const CASE =
...
export const BatteryFullIcon = forwardRef<IconHandle, IconProps>(
  function BatteryFullIcon({ size = 28, style, ...props }, ref) {
