...
@Composable internal fun SettingsScreen(s:StudioState){
    ...
    if(s.languagePage){
        Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(bottom=20.dp)){
            ...
            (listOf("system" to s.tr("systemLanguage"))+Languages.codes.zip(Languages.names)).forEach{(code,name)->
                BrainListCard(onClick={save(p.copy(language=code))},modifier=Modifier.padding(bottom=8.dp)){
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(bottom=20.dp)){
        ...
        BrainPanel(Modifier.fillMaxWidth(),padding=18.dp){
            ...
            BrainSlider(quality,{quality=it},0f..3f,2,s.tr("quality"),{save(p.copy(quality=quality.toInt()))})
            ...
            BrainSlider(speed,{speed=it},1f..2f,3,s.tr("savedSpeed"),{save(p.copy(savedSpeed=(kotlin.math.round(speed*4)/4).toDouble()))})
        ...
        BrainListCard(onClick={s.languagePage=true}){
