...
private val LightPaper = Color(0xFFF7F5EE)      // почти белый, едва тёплый/желтоватый
private val LightSurface = Color(0xFFFFFEFA)
private val DarkPaper = Color(0xFF191715)       // почти чёрный, едва коричневый
...
fun StudioTheme(theme: String, content: @Composable () -> Unit) {
    ...
    val textFamily = FontFamily(
        Font(Res.font.wix_madefor_text_regular, FontWeight.Normal),
        Font(Res.font.wix_madefor_text_medium, FontWeight.Medium),
        Font(Res.font.wix_madefor_text_semibold, FontWeight.SemiBold),
    )
    val displayFamily = FontFamily(Font(Res.font.wix_madefor_display_semibold, FontWeight.SemiBold))
    ...
    val colors = if (!dark) {
        lightColorScheme(
            ...
            outline = Color(0xFFBAB4AA),
            error = Color(0xFFA33A35),
        )
    } else {
        darkColorScheme(
            ...
            onSurfaceVariant = Color(0xFFB4ADA5),
            outline = Color(0xFF625B54),
            error = Color(0xFFEE9A92),
        )
    }
    ...
    // Размеры проверены под мобильный viewport: основной текст 15–17sp, навигация >=11sp,
    // крупные заголовки не превышают 46sp и не требуют desktop-only ширины.
    val typography = Typography(
        displayLarge = display(46, 50),
        displayMedium = display(38, 43),
        displaySmall = display(32, 37),
        headlineLarge = display(28, 34),
        headlineMedium = display(24, 30),
        headlineSmall = display(20, 26),
        titleLarge = text(20, 27, FontWeight.Medium),
        titleMedium = text(17, 24, FontWeight.Medium),
        titleSmall = text(15, 21, FontWeight.Medium),
        bodyLarge = text(17, 26),
        bodyMedium = text(15, 22),
        bodySmall = text(13, 19),
        labelLarge = text(15, 20, FontWeight.Medium),
...
/** Экранные имена сохранены компактными, но все контролы идут только через Kasha UI. */
@Composable
